...
```text
> toggleBookmark      # mark the current line          (Ctrl-d)
F2                   # jump to next bookmark
> prevBookmark        # jump to previous (no default key)
```
...
|--------------------|------------------|--------------------------------------------------------|
| `toggleBookmark`   | `Ctrl-d`         | Mark/unmark the current line                           |
| `nextBookmark`     | `F2`             | Jump to the next bookmark in the active list           |
| `prevBookmark`     | —                | Jump to the previous bookmark in the active list       |
...
| `clearBookmarks`   | —                | Clear all bookmarks in the active list (with confirm)  |
| `listBookmarks`    | —                | Open the picker for the current buffer                 |
...
2. Walk the diff; `> toggleBookmark` (`Ctrl-d`) on each line worth revisiting, and `> nameBookmark` to note *why*.
...
- **`F2` does nothing** — some terminals intercept function keys. Bind the commands to other keys in `bindings.json` (see above).
